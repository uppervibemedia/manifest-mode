...
import { ChevronLeft, Send, Sparkles, Crown, Lock } from "lucide-react";
import AppLayout from "@/components/layout/AppLayout";
import CoachMessage from "@/components/coach/CoachMessage";
import AIInsightsPanel from "@/components/coach/AIInsightsPanel";
...
export default function FutureSelfCoach() {
  ...
  const loadContext = async () => {
    ...
    // Only set welcome message for premium users
    if (userTier === "premium") {
  ...
  const isSafetyTopic = (text) => SAFETY_KEYWORDS.some(kw => text.toLowerCase().includes(kw));
  ...
  if (tier === "free") {
    return (
      <AppLayout>
        <div className="px-5 pt-12 pb-6 flex flex-col items-center justify-center min-h-[80vh] text-center">
          <div className="w-16 h-16 rounded-2xl glass-card border border-border flex items-center justify-center mb-5">
            <Lock className="w-7 h-7 text-muted-foreground" />
          </div>
          <p className="text-xs uppercase tracking-widest text-primary/70 font-medium mb-2">Premium Feature</p>
          <h2 className="font-playfair text-2xl font-semibold mb-3">Future Self Coach</h2>
          <p className="text-sm text-muted-foreground mb-8 max-w-xs leading-relaxed">
            AI coaching is available on Plus and Premium plans. Upgrade to start receiving personalized guidance tied to your Blueprint and Score.
          </p>
  ...
  // SUPPORTER/PLUS tier — AI Insights only
  if (tier === "supporter") {
    return (
      <AppLayout>
        <AIInsightsPanel
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5 flex-1">
              ...
              <div>
                ...
                <p className="text-[10px] text-primary/70 mt-0.5">Premium · Personalized · Unlimited</p>
              </div>
            </div>
            {context?.score && (
